...
bool operator==(const Cube &lhs, const Cube &rhs) {
    for (int i = 0; i < NUM_OF_SIDES; i++) {
        for (int j = 0; j < NUM_OF_SQUARES; j++) {
            if (lhs.getSide(static_cast<Side::Colors>(i)).getSquare(static_cast<Side::SquarePosition>(j)) !=
                rhs.getSide(static_cast<Side::Colors>(i)).getSquare(static_cast<Side::SquarePosition>(j))) {
                return false;
            }
        }
    }
...
void Cube::printCube() const {
    ...
    for (int i = 0; i < col_of_squares; i++) {
        for (int j = 0; j < row_of_squares * NUM_OF_BODY_SIDES; j++) {
            ...
            switch (
                cube_side_order[(j / row_of_squares)]->getSquare(static_cast<SquarePosition>((j % row_of_squares) + (i * SIDE_LENGTH)))) {
            case Yellow:
                std::cout << "\033[93m";
                break;
            case White:
...
void Cube::inverse_move(Side *side, const std::array<SquarePosition, SIDE_LENGTH> &original_squares,
    ...
    for (int i = 0; i < NUM_OF_SIDES; i++) {
        for (int j = 0; j < relevant_squares.size(); j++) {
            if (getSide(static_cast<Colors>(i)).getSquare(relevant_squares[j]) == White &&
                &getSide(static_cast<Colors>(i)) != &getSide(Yellow)) {
        ...
        for (int j = 0; j < relevant_squares.size(); j++) {
            if (getSide(static_cast<Colors>(i)).getSquare(relevant_squares[j]) == White) {
                if (&getSide(static_cast<Colors>(i)) != &getSide(White)) {
                    return {i, static_cast<SquarePosition>(relevant_squares[j])};
                } else if (relevant_squares[j] == TopLeftCorner && (front->getSquare(BottomLeftCorner) != front->getSquare(Middle) ||
                                                                    left->getSquare(BottomLeftCorner) != left->getSquare(Middle))) {
                    return {i, static_cast<SquarePosition>(relevant_squares[j])};
                } else if (relevant_squares[j] == TopRightCorner && (front->getSquare(BottomRightCorner) != front->getSquare(Middle) ||
                                                                     right->getSquare(BottomLeftCorner) != right->getSquare(Middle))) {
                    return {i, static_cast<SquarePosition>(relevant_squares[j])};
                } else if (relevant_squares[j] == BottomLeftCorner && (back->getSquare(BottomRightCorner) != back->getSquare(Middle) ||
                                                                       left->getSquare(BottomLeftCorner) != left->getSquare(Middle))) {
                    return {i, static_cast<SquarePosition>(relevant_squares[j])};
                } else if (relevant_squares[j] == BottomRightCorner &&
                           (back->getSquare(BottomLeftCorner) != back->getSquare(Middle) ||
                            right->getSquare(BottomRightCorner) != right->getSquare(BottomRightCorner))) {
                    return {i, static_cast<SquarePosition>(relevant_squares[j])};
                }
            }
        }
...
void Cube::whiteCross() {
    ...
    for (int i = 0; i < NUM_OF_BODY_SIDES; i++) {
        if (relevant_sides[i] == left) {
            turnTopUntil([&]() {
                return (relevant_sides[i]->getSquare(TopEdge) != relevant_sides[i]->getSquare(Middle) || up->getSquare(LeftEdge) != White);
            });
            left_up();
            left_up();
        } else if (relevant_sides[i] == front) {
            turnTopUntil([&]() {
                return (relevant_sides[i]->getSquare(TopEdge) != relevant_sides[i]->getSquare(Middle) ||
                        up->getSquare(BottomEdge) != White);
            });
            front_left();
            front_left();
        } else if (relevant_sides[i] == right) {
            turnTopUntil([&]() {
                return (relevant_sides[i]->getSquare(TopEdge) != relevant_sides[i]->getSquare(Middle) || up->getSquare(RightEdge) != White);
            });
            right_up();
            right_up();
        } else if (relevant_sides[i] == back) {
            turnTopUntil([&]() {
                return (relevant_sides[i]->getSquare(TopEdge) != relevant_sides[i]->getSquare(Middle) || up->getSquare(TopEdge) != White);
            });
            back_left();
            back_left();
        }
...
void Cube::whiteCorners() {
    PieceLocation white_corner{};
    while ((white_corner = find_missing_white_corner()), white_corner.face != -1 || white_corner.pos != Middle) {
        if (&getSide(static_cast<Colors>(white_corner.face)) == up) {
            if (white_corner.pos == TopLeftCorner) {
                if (back->getSquare(TopRightCorner) != back->getSquare(Middle)) {
        ...
        // if (&getSide(static_cast<Colors> (white_corner.face)) == down) {
        //     if (white_corner.pos == TopLeftCorner && front->getSquare(BottomLeftCorner) != front->getSquare(Middle)) {
        //
        //     }
    }
}
